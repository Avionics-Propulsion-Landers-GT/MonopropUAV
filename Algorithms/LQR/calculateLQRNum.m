function [K,S,P] = calculateLQRNum(Q, R, t, m_num, state, input, rc_num, rt_num, f_num, cDrag_num, areaVar_num, inertia_num, inertia_s_num, inertia_a_num, inertia_b_num)
    %% Initialization
    % State is a 12x1 column vector
    % input is a 1x7 row vector
    ...
    % A = double(subs(A, [m full_state full_input rc rt inertia inertia_s inertia_a inertia_b f cDrag areaVar], ...
    %     [m_num state input rc_num rt_num inertia_num inertia_s_num inertia_a_num inertia_b_num f_num cDrag_num areaVar_num]));
    % B = double(subs(A, [m full_state full_input rc rt inertia inertia_s inertia_a inertia_b f cDrag areaVar], ...
    %     [m_num state input rc_num rt_num inertia_num inertia_s_num inertia_a_num inertia_b_num f_num cDrag_num areaVar_num]));
    A = subs(A, [m f cDrag areaVar], [m_num f_num cDrag_num areaVar_num]);
    A = subs(A, full_input, input);
    A = subs(A, full_state, state);
    A = subs(A, rc, rc_num);
    A = subs(A, rt, rt_num);
    A = subs(A, [inertia inertia_s inertia_a inertia_b], [inertia_num inertia_s_num inertia_a_num inertia_b_num]);

    B = subs(B, [m f cDrag areaVar], [m_num f_num cDrag_num areaVar_num]);
    B = subs(B, full_input, input);
    B = subs(B, full_state, state);
    B = subs(B, rc, rc_num);
    B = subs(B, rt, rt_num);
    B = subs(B, [inertia inertia_s inertia_a inertia_b], [inertia_num inertia_s_num inertia_a_num inertia_b_num]);
