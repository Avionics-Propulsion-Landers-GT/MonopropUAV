function monocopter_dynamics_spiral()
    ...
    Kp_pos = [3.5; 3.5; 0.5];    % Proportional gains for X, Y, Z
    Ki_pos = [0; 0; 0];    % Integral gains for X, Y, Z
    Kd_pos = [2.0; 2.0; 1.5];    % Derivative gains for X, Y, Z

    % PID gains for attitude control
    Kp_att = [9.0; 9.0; 9.0];  % Proportional gains for Roll, Pitch, Yaw
    Kd_att = [1.0; 1.0; 1.0];  % Derivative gains for Roll, Pitch, Yaw

    % Initialize desired trajectory
    % Parameters for the spiral trajectory
    num_spirals = 3;   % Number of spirals
    z_max = 100;       % Maximum height (z position)
    z_min = 0;        % Starting height (z position)
    radius = 30;       % Radius of the spiral
    
    % Generate the desired trajectory
    position_desired = zeros(3, num_steps);
    for i = 1:num_steps
        t_norm = (i - 1) / (num_steps - 1); % Normalized time from 0 to 1
        theta = num_spirals * 2 * pi * t_norm; % Angle for spiral
        z_desired = z_min + (z_max - z_min) * t_norm; % Linearly increase height
    
        % Calculate the desired X and Y positions for the spiral
        x_desired = radius * cos(theta); % Circular motion in X
        y_desired = radius * sin(theta); % Circular motion in Y
    
        position_desired(:, i) = [x_desired; y_desired; z_desired];
    end
...
function plot_results(position_history, velocity_history, acceleration_history, quaternion_history, angular_velocity_history, error_history, dt, t, position_desired)
    ...
    eulers_calc = zeros(3, size(quaternion_history,2));
    for i = 1:size(quaternion_history,2)
        
        w = quaternion_history(1,i)/norm(quaternion_history(:,i));
        x = quaternion_history(2,i)/norm(quaternion_history(:,i));
        y = quaternion_history(3,i)/norm(quaternion_history(:,i));
        z = quaternion_history(4,i)/norm(quaternion_history(:,i));
        
        roll = atan2(2*(w*x + y*z),1-2*(x^2+y^2));
        pitch = -pi/2 + 2*atan2(sqrt(1+2*(w*y - x*z)),sqrt(1-2*(w*y - x*z)));
        yaw = atan2(2*(w*z + x*y), 1-2*(y^2+z^2));
        [roll; pitch; yaw]
        eulers_calc(:,i) = [roll; pitch; yaw];
        
        euler = quat2eul([w,x,y,z],"XYZ")
        eulers(:,i) = euler;
