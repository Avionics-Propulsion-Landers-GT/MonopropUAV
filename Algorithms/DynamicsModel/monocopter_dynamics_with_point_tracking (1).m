function monocopter_dynamics_with_point_tracking()
    % Simulation parameters
    dt = 0.01;  % Time step (seconds)
    t_end = 100;  % Simulation time (seconds)
    ...
    thrust_dir_body = [1; 2; 5];  % Thrust in the body frame (along z-axis)
    ...
    % Target point definition (fixed in this case)
    target_point = [10; 10; 2];  % Target position (m)

    % PID controller gains for position (example)
    Kp_pos = 1;  % Proportional gain
    Ki_pos = 0.5;  % Integral gain
    Kd_pos = 0.3;  % Derivative gain
    ...
    A_wind = 10;  % Amplitude of the wind (m/s)
    omega_wind = 1;  % Frequency of the wind oscillation (rad/s)
    phase_shift_x = pi/4;  % Phase shift for wind in x-direction
    phase_shift_y = 1;  % Phase shift for wind in y-direction
    phase_shift_z = 1;  % Phase shift for wind in z-direction

    % Main simulation loop
    for step = 1:num_steps
        t = (step - 1) * dt;  % Current time

        % Define the target point (fixed for now, can be updated dynamically)
        reference_pos = target_point;

        % Calculate the position error (difference between current position and target)
        error_pos = reference_pos - r;
        error_history(:, step) = error_pos;  % Store error for analysis

        % PID control for following the target point
        ...
        % Calculate desired yaw and pitch based on the direction towards the target point
        ...
        desired_pitch = atan2(desired_direction(3), sqrt(desired_direction(1)^2 + desired_direction(2)^2));  % Pitch towards the target
        ...
        omega_dot = euler_equations(omega, M, I);  % Use the provided euler_equations function
    ...
    plot_results_with_arrows_and_error(position_history, velocity_history, quaternion_history, angular_velocity_history, error_history, target_point, yaw_pitch_history, dt);
end
...
function plot_results_with_arrows_and_error(position_history, velocity_history, quaternion_history, angular_velocity_history, error_history, target_point, yaw_pitch_history, dt)
    ...
    plot3(target_point(1), target_point(2), target_point(3), 'go', 'MarkerSize', 10, 'MarkerFaceColor', 'g');
    ...
    grid on;
    title('Monoprop 3D Trajectory and Target Point');
    xlabel('X Position (m)');
    ylabel('Y Position (m)');
    zlabel('Z Position (m)');
    legend('Actual Trajectory', 'Target Point', 'Velocity Arrows');
